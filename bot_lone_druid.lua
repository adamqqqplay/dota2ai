...
function ConsiderReturn(hMinionUnit)
	...
	if bearState == "retreat" or RetreatDesire > 0 or abilityFG:GetLevel() < 2 or not abilityRT:IsFullyCastable() or
		abilityRT:IsHidden() then
...
function ConsiderRetreat(hMinionUnit)
	...
	if not (mode == BOT_MODE_ATTACK or mode == BOT_MODE_DEFEND_ALLY) and
		(hMinionUnit:WasRecentlyDamagedByAnyHero(2.0) or
