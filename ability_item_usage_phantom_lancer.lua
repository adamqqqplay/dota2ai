	...
	then
		...
		then
			...
			then
				...
				then
					if ( CanCast[abilityNumber]( npcEnemy ) and GetUnitToUnitDistance(npcBot,npcEnemy)>CastRange-100 and GetUnitToUnitDistance(npcBot,npcEnemy)<1000)
					then
...
Consider[3] = function()
	...
	if npcBot:GetActiveMode() == BOT_MODE_LANING then
		...
		local weakestCreep, creepHealth = utility.GetWeakestUnit(creeps)
		if creepHealth <= weakestCreep:GetActualIncomingDamage(npcBot:GetAttackDamage()*1.05, DAMAGE_TYPE_PHYSICAL) and GetUnitToUnitDistance(npcBot, weakestCreep) <= 250 then
			return true
		end
	end
	return false
