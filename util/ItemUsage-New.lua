...
local function IsStuck(npcBot)
    if npcBot.stuckLoc ~= nil and npcBot.stuckTime ~= nil then
        ...
        if npcBot:GetCurrentActionType() == BOT_ACTION_TYPE_MOVE_TO and attackTarget == nil and EAd > 2200 and TAd > 2200
            and #Et == 0 and #At == 0 and DotaTime() > npcBot.stuckTime + 5.0 and
...
local function ShouldTP()
    ...
    elseif mode == BOT_MODE_DEFEND_ALLY and modDesire >= BOT_MODE_DESIRE_MODERATE and
        role.CanBeSupport(npcBot:GetUnitName()) == true and #enemies == 0 then
...
function M.ItemUsageThink()
        ...
        if DotaTime() > 0 then
            ...
            do
                local weakestAlly = nearbyAllies:Filter(function(it)
                    return fun1:GetHealthPercent(it) < 0.25 and not it:WasRecentlyDamagedByAnyHero(1.5) and
                        it:GetActiveMode() ~= BOT_MODE_RETREAT and
                        (
                        tableNearbyEnemyHeroes2 or it:HasModifier "modifier_templar_assassin_reflection_absorb" or
                            fun1:HasAnyModifier(it, fun1.IgnoreDamageModifiers))
    ...
    if icl ~= nil and icl:IsFullyCastable() and npcBot:DistanceFromFountain() > 1000 then
        ...
        if DotaTime() > 0 then
            ...
            do
                local weakestAlly = nearbyAllies:Filter(function(it)
                    return fun1:GetManaPercent(it) < 0.4 and not it:WasRecentlyDamagedByAnyHero(3) and
                        it:GetActiveMode() ~= BOT_MODE_RETREAT and
                        (
                        tableNearbyEnemyHeroes2 or it:HasModifier "modifier_templar_assassin_reflection_absorb" or
                            fun1:HasAnyModifier(it, fun1.IgnoreDamageModifiers))
    ...
    if ff ~= nil and ff:IsFullyCastable() and notBlasted then
        if npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
            healthPercent < 0.15 and npcBot:WasRecentlyDamagedByAnyHero(3) then
    ...
    if pb ~= nil and pb:IsFullyCastable() then
        if (
            npcBot:GetActiveMode() == BOT_MODE_ATTACK or npcBot:GetActiveMode() == BOT_MODE_RETREAT or
                ...
                npcBot:GetActiveMode() == BOT_MODE_GANK or npcBot:GetActiveMode() == BOT_MODE_DEFEND_ALLY) and
            not fun1:IsSeverelyDisabled(npcBot) then
    ...
    if bt ~= nil and bt:IsFullyCastable() then
        if npcBot:GetActiveMode() == BOT_MODE_ATTACK or npcBot:GetActiveMode() == BOT_MODE_ROAM or
            npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or npcBot:GetActiveMode() == BOT_MODE_GANK or
            npcBot:GetActiveMode() == BOT_MODE_DEFEND_ALLY then
            if npcTarget ~= nil and npcTarget:IsHero() and fun1:NormalCanCast(npcTarget) and
                GetUnitToUnitDistance(npcTarget, npcBot) < 900 then
    ...
    if sc ~= nil and sc:IsFullyCastable() then
        ...
        do
            local ally = nearbyAllies:Filter(fun1.PhysicalCanCastFunction):Filter(function(t)
                return not t:HasModifier("modifier_item_medallion_of_courage_armor_addition") and
                    not t:HasModifier("modifier_item_solar_crest_armor_addition")
        ...
        if npcBot:GetActiveMode() == BOT_MODE_ATTACK or npcBot:GetActiveMode() == BOT_MODE_ROAM or
            npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or npcBot:GetActiveMode() == BOT_MODE_GANK or
            npcBot:GetActiveMode() == BOT_MODE_DEFEND_ALLY then
            if npcTarget ~= nil and npcTarget:IsHero() and GetUnitToUnitDistance(npcTarget, npcBot) < 900 and
                not npcTarget:HasModifier("modifier_item_medallion_of_courage_armor_addition") and
                not npcTarget:HasModifier("modifier_item_medallion_of_courage_armor_reduction") then
        ...
        do
            local ally = nearbyAllies:Filter(function(it)
                return (fun1:GetHealthPercent(it) < 0.35 and #tableNearbyEnemyHeroes > 0 or fun1:IsSeverelyDisabled(it))
                    and fun1:AllyCanCast(it)
            end)
            if ally then
                M.UseItemOnEntity(npcBot, sc, Ally)
                return
    ...
    if se ~= nil and se:IsFullyCastable() then
        if npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
            tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0 then
            M.UseItemNoTarget(npcBot, se)
            return
        end
        if npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or
            npcBot:GetActiveMode() == BOT_MODE_GANK then
            if npcTarget and fun1:MayNotBeIllusion(npcBot, npcTarget) and npcTarget:IsHero() and
                GetUnitToUnitDistance(npcTarget, npcBot) > 1000 and GetUnitToUnitDistance(npcTarget, npcBot) < 2500 then
                M.UseItemNoTarget(npcBot, se)
                return
            end
        end
    ...
    if lotus ~= nil and lotus:IsFullyCastable() then
        if (healthPercent < 0.45 and tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0) or
            npcBot:IsSilenced() or
            (tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes >= 3 and healthPercent < 0.75) then
        ...
        for _, Ally in pairs(Allies) do
            if (
                Ally:GetHealth() / Ally:GetMaxHealth() < 0.35 and tableNearbyEnemyHeroes ~= nil and
                    #tableNearbyEnemyHeroes > 0) or fun1:IsOrGoingToBeSeverelyDisabled(Ally) then
    ...
    local itemDagon = fun1:Aggregate(GetItemIfNotImplemented("item_dagon"), fun1:Range(2, 5),
        function(seed, dagonLevelIndex)
    ...
    if glimmer and glimmer:IsFullyCastable() then
        if (healthPercent < 0.45 and (tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0)) or
            (tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes >= 3 and healthPercent < 0.65) then
            M.UseItemOnEntity(npcBot, glimmer, npcBot)
            return
        end
    end
    local hod = GetItemIfNotImplemented("item_helm_of_the_overlord") or
        GetItemIfNotImplemented("item_helm_of_the_dominator")
    ...
    if glimer ~= nil and glimer:IsFullyCastable() then
        ...
        for _, Ally in pairs(Allies) do
            if (
                Ally:GetHealth() / Ally:GetMaxHealth() < 0.35 and tableNearbyEnemyHeroes ~= nil and
                    #tableNearbyEnemyHeroes > 0 and fun1:NormalCanCast(Ally)) or
                (fun1:IsOrGoingToBeSeverelyDisabled(Ally) and fun1:NormalCanCast(Ally)) then
    ...
    if satanic ~= nil and satanic:IsFullyCastable() and notBlasted then
        if healthPercent < 0.50 and tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0 and
    ...
    local function NotSuitableForGuardianGreaves(t)
        return fun1:AllyCanCast(t) and not t:HasModifier("modifier_ice_blast") and
            not t:HasModifier("modifier_item_mekansm_noheal") and
            not t:HasModifier("modifier_item_guardian_greaves_noheal")
    end
    ...
    if stick ~= nil and stick:IsFullyCastable() and stick:GetCurrentCharges() > 0 and notBlasted then
        if DotaTime() > 0 then
            ...
            if (
                (healthPercent < 0.4 or manaPercent < 0.2) and #tableNearbyEnemyHeroes >= 1 and
                    GetItemCharges(npcBot, "item_magic_stick") >= 1) or
                ((healthPercent < 0.7 and manaPercent < 0.7) and GetItemCharges(npcBot, "item_magic_stick") >= 7) then
    ...
    if wand ~= nil and wand:IsFullyCastable() and wand:GetCurrentCharges() > 0 and notBlasted then
        if DotaTime() > 0 then
            ...
            if (
                ...
                ((healthPercent < 0.7 and manaPercent < 0.7) and GetItemCharges(npcBot, "item_magic_wand") >= 12) then
    ...
    if holyLocket ~= nil and holyLocket:IsFullyCastable() and notBlasted then
        if DotaTime() > 0 then
            ...
            if (
                ...
                ((healthPercent < 0.7 and manaPercent < 0.7) and GetItemCharges(npcBot, "item_holy_locket") >= 12) then
    ...
    if bottle ~= nil and bottle:IsFullyCastable() and notBlasted then
        ...
        if GetItemCharges(npcBot, "item_bottle") > 0 and not npcBot:HasModifier("modifier_bottle_regeneration") then
            if ((healthPercent < 0.65 and manaPercent < 0.45) or healthPercent < 0.4 or manaPercent < 0.2) and
    ...
    if cyclone ~= nil and cyclone:IsFullyCastable() then
        if npcTarget ~= nil and
            (
            npcTarget:IsChanneling() and not fun1:IsOrGoingToBeSeverelyDisabled(npcTarget) or
                fun1:CannotBeKilledNormally(npcTarget)) and fun1:NormalCanCast(npcTarget) and
            GetUnitToUnitDistance(npcBot, npcTarget) < 775 then
    ...
    if metham ~= nil and metham:IsFullyCastable() then
        ...
        elseif npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or
            ...
            if npcTarget ~= nil and npcTarget:IsHero() and fun1:NormalCanCast(npcTarget) and
                ...
                fun1:IsOrGoingToBeSeverelyDisabled(true, npcTarget) == true then
    ...
    if sv ~= nil and sv:IsFullyCastable() and sv:GetCurrentCharges() > 0 then
        ...
        if npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or
            npcBot:GetActiveMode() == BOT_MODE_DEFEND_ALLY or npcBot:GetActiveMode() == BOT_MODE_ATTACK then
            if npcTarget ~= nil and npcTarget:IsHero() and fun1:MayNotBeIllusion(npcBot, npcTarget) and
                fun1:NormalCanCast(npcTarget) and GetUnitToUnitDistance(npcBot, npcTarget) < 900 and
                npcTarget:HasModifier("modifier_item_spirit_vessel_damage") == false and
                npcTarget:GetHealth() / npcTarget:GetMaxHealth() < 0.65 and
                not npcTarget:HasModifier("modifier_ice_blast") then
            ...
            for _, Ally in pairs(Allies) do
                if not Ally:IsIllusion() and Ally:HasModifier("modifier_item_spirit_vessel_heal") == false and
                    fun1:NormalCanCast(Ally) and Ally:GetHealth() / Ally:GetMaxHealth() < 0.35 and
                    #tableNearbyEnemyHeroes == 0 and Ally:WasRecentlyDamagedByAnyHero(2.5) == false and
                    not Ally:HasModifier("modifier_ice_blast") then
    ...
    if nullifier ~= nil and nullifier:IsFullyCastable() then
        if npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or
            ...
            if npcTarget ~= nil and npcTarget:IsHero() and fun1:NormalCanCast(npcTarget) and
                GetUnitToUnitDistance(npcBot, npcTarget) < 800 and
                npcTarget:HasModifier("modifier_item_nullifier_mute") == false then
    ...
    if sentry ~= nil and sentry:IsFullyCastable() and sentry:IsCooldownReady() then
        ...
        if HaveWard == false then
            ...
            if npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_TOP or npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_MID or
                npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_BOT and #NearbyTowers2 == 0 and #NearbyTowers > 0 then
                M.UseItemOnLocation(npcBot, sentry, npcBot:GetXUnitsInBehind(300))
            end
            if npcBot:GetActiveMode() == BOT_MODE_DEFEND_TOWER_TOP or npcBot:GetActiveMode() == BOT_MODE_DEFEND_TOWER_MID
                or npcBot:GetActiveMode() == BOT_MODE_DEFEND_TOWER_BOT and #NearbyTowers3 == 0 then
