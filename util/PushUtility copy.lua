...
local function GetSafeLocationToEnemyBuilding(npc, building)
	...
	if(MinDamageHeroID==npc:GetPlayerID() and #Allys>=3 and #enemys~=0)
	then
		attackRange = 800
	end
...
function tryTP( npcBot,lane )
	...
	if DistanceToFront > 4000 then
		if tp and DistanceToFront - distBuilding > 3000 and DistanceToLane >3000 then
...
local function getMyTarget(npcBot,lane,TargetLocation)
	...
	if creeps~=nil and #creeps>=1 
	-- and (GetUnitToLocationDistance(npcBot,TargetLocation)<=200 or TowerDistance<=800)
	then
		-- if(GetUnitToLocationDistance(creeps[1],EnemySpawnLocation)<DistanceTowerToEnemyHome-MaxDiveLength)	--This location is diving tower, do not make yourself in dangerous.
		-- then
			return creeps[1]
		-- end
	end
...
local function OkToBeAttackedByTower(npc, targetBuilding) 
	...
	if hpPercent >= 0.9 and npc:GetLevel() >= 7 then
		return true
	end
	local rate = 0
	...
	if health >= 1000 and hpPercent >= 0.8 - enemyCount * 0.1 then
...
function UnitPushLaneThink(npcBot,lane)
	...
	local EnemyTower = GetNearestBuilding(GetOpposingTeam(), front)
	if EnemyTower then
		print(getShortName(npcBot).." at "..AbilityExtensions:ToStringVector(npcBot:GetLocation()).." enemy tower at "..AbilityExtensions:ToStringVector(EnemyTower:GetLocation()))
		print("lane front at "..AbilityExtensions:ToStringVector(front))
	end
	local TowerDistance = GetUnitToUnitDistance(npcBot,EnemyTower)
	...
	local enemys = npcBot:GetNearbyHeroes(1400,true,BOT_MODE_NONE)
	...
	elseif goodSituation==false then
		StepBack( npcBot )
		print(getShortName(npcBot).." situation is not good")
	elseif npcBot:WasRecentlyDamagedByTower(0.4) or EnemyTower and EnemyTower:GetAttackTarget() == npcBot then		--if I'm under attck of tower, then try to avoid attack
		local transfer = not OkToBeAttackedByTower(npcBot, EnemyTower)
		local stepBack = true
		if EnemyTower:GetAttackDamage() > 20 and not EnemyTower:HasModifier "modifier_fountain_glyph" then
			if enemyCount >= 1 then
				if transfer then
					stepBack = not TransferHatred(npcBot)
				end
			end
		end
		if stepBack then
			...
			if npcBot:GetAttackTarget() ~= EnemyTower then
				npcBot:Action_AttackUnit(EnemyTower, false) 
			end
		end
		print(getShortName(npcBot).." step back")
	elseif GetUnitToLocationDistance(npcBot,TargetLocation)>=MinDelta then
		print(getShortName(npcBot).." move ha")
		npcBot:Action_MoveToLocation(TargetLocation)
	elseif target then
		print(getShortName(npcBot).." just attack "..target:GetUnitName())
		npcBot:Action_AttackUnit( target, true )
	elseif TowerDistance <= 1200 then
		print(getShortName(npcBot).." tower distance too high")
		...
		print("towerdistance = "..tostring(TowerDistance)..", towerdmg = "..EnemyTower:GetAttackDamage()..", creepattack = "..tostring(CreepAttackTower))
		npcBot:Action_MoveToLocation(TargetLocation)
	end
	local t1 = npcBot:GetAttackTarget()
	if t1 then
		print(getShortName(npcBot).." is attacking "..t1:GetUnitName())
	end
	return true
end
...
function GetNearestBuilding(team, location)
	return GetAllBuilding(team, location):Filter(function(t) 
		return not t:IsInvulnerable()
	end):MinKey(function(t)
		return GetUnitToLocationDistanceSqr(t, location) 
	end) or GetAncient(team)
end
...
function GetNearestBuilding(team, location)
	...
	for k,v in pairs(buildings) do
		if not v:IsInvulnerable() then
			...
			print(AbilityExtensions:ToStringVector(v:GetLocation()).." distance to "..v:GetUnitName()..AbilityExtensions:ToStringVector(location).." is "..DistanceToFront)
	...
	if nearestBuilding then
		print(AbilityExtensions:ToStringVector(nearestBuilding:GetLocation()).." dis = "..minDist)
...
function GetAllBuilding( team,location )
	return A.Linq.Range(0, 10):Map(function(t)
		return GetTower(team, t) 
	end):Concat(
		(A.Linq.Range(0, 5):Map(function(t) 
		return GetBarracks(team, t)
	end)
	):Concat(
		A.Linq.Range(0, 4):Map(function(t) 
...
function TransferHatred( unit )
	...
	for k,creep in pairs(creeps) do
		if NotNilOrDead(creep) and NotNilOrDead(tower) then
			print(unit:GetUnitName()..": transfer hatred "..creep:GetUnitName()..", push desire = "..unit:GetActiveModeDesire())
