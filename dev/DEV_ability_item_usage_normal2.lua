...
local function GetComboMana()
	...
	if AbilitiesReal[1]:GetLevel() < 1 or AbilitiesReal[2]:GetLevel() < 1 or AbilitiesReal[3]:GetLevel() < 1 or
		AbilitiesReal[4]:GetLevel() < 1
	then
		tempComboMana = 300;
	end
		...
		then
			for i = 1, 4, 1 do
				...
				then
					...
					then
						npcBot:ActionImmediate_Chat("try to use skill " ..
							i .. " at " .. castTarget[i]:GetUnitName() .. " Desire= " .. castDesire[i], true)
					else
...
function Consider2() --Target AOE Ability Example
	...
	if (WeakestCreep ~= nil)
	then
		if ((ManaPercentage > 0.5 or npcBot:GetMana() > ComboMana) and
			GetUnitToUnitDistance(npcBot, WeakestCreep) >= AttackRange + 100)
		then
			if (CreepHealth <= WeakestCreep:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL))
			then
...
function Consider3() --Location AOE Example
				...
				if (
					HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) or
						...
						HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and
							npcBot:GetMana() > ComboMana))
				then
					return BOT_ACTION_DESIRE_HIGH, WeakestEnemy:GetExtrapolatedLocation(0.5);
				end
			end
		end
	end
	...
	then
		for _, npcEnemy in pairs(enemys) do
			...
			then
				if (CanCast[abilityNumber](npcEnemy))
				then
					return BOT_ACTION_DESIRE_MODERATE, npcEnemy:GetExtrapolatedLocation(0.5);
				end
			end
		end
	end
	...
	then
		...
		then
			if (CanCast[abilityNumber](npcEnemy))
			then
				return BOT_ACTION_DESIRE_HIGH, npcEnemy:GetExtrapolatedLocation(0.5);
			end
		end
	end
