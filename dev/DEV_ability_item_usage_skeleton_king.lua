			...
			then
				if (
					HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) or
						HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_ALL))
				then
...
Consider[3] = function() --Location AOE Example
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM and npcBot:HasModifier("modifier_skeleton_king_mortal_strike_summon")
		and npcBot:GetModifierStackCount("modifier_skeleton_king_mortal_strike_summon") > 2 and ManaPercentage > 0.45)
	then
	...
	then
		if (#creeps2 >= 3 and #tower >= 1 and npcBot:HasModifier("modifier_skeleton_king_mortal_strike_summon")
			and npcBot:GetModifierStackCount("modifier_skeleton_king_mortal_strike_summon") > 2 and ManaPercentage > 0.45)
		then
			return BOT_ACTION_DESIRE_MODERATE
		end
	...
	then
		...
		if (npcEnemy ~= nil and npcBot:HasModifier("modifier_skeleton_king_mortal_strike_summon")
			and npcBot:GetModifierStackCount("modifier_skeleton_king_mortal_strike_summon") > 2 and ManaPercentage > 0.45)
		then
	...
	then
		...
		if (npcEnemy ~= nil and npcBot:HasModifier("modifier_skeleton_king_mortal_strike_summon")
			and npcBot:GetModifierStackCount("modifier_skeleton_king_mortal_strike_summon") > 2)
		then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
