...
function GetBotClosestRune()
	local cDist = 100000;
	local cRune = -1;
	for _, r in pairs(ListRune) do
		local rLoc = GetRuneSpawnLocation(r);
		if not IsHumanPlayerNearby(rLoc) and not IsPingedByHumanPlayer(rLoc) and not IsThereMidlaner(rLoc) and
			not IsThereCarry(rLoc) and IsTheClosestOne(rLoc)
		then
...
function IsPingedByHumanPlayer(runeLoc)
	...
	for _, p in pairs(listPings) do
		if p ~= nil and not p.normal_ping and GetDistance(p.location, runeLoc) < 1200 and dist2 < 1200 and
