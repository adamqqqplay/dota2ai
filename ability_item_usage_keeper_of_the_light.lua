...
local ConsiderIlluminate = function(abilityNumber)
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) and #allys < 3 then
		local locationAoE = npcBot:FindAoELocation(true, false, npcBot:GetLocation(), CastRange, Radius, ChannelTime / 2,
			Damage);
	...
	then
		...
		then
			local locationAoE = npcBot:FindAoELocation(true, true, npcBot:GetLocation(), CastRange, Radius, ChannelTime / 2,
				Damage);
...
function AbilityUsageThink()
	...
	if npcBot:IsCastingAbility() or npcBot:IsUsingAbility() or npcBot:IsInvulnerable() or npcBot:IsChanneling() or
		npcBot:IsSilenced() or npcBot:HasModifier("modifier_doom_bringer_doom") or npcBot:NumQueuedActions() > 0 then return end
