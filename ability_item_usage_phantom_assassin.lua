...
local CanCast = { function(t)
	return AbilityExtensions:NormalCanCast(t, false, DAMAGE_TYPE_PHYSICAL, true, true, true) and
		not AbilityExtensions:HasAbilityRetargetModifier(npcBot)
end, function(t)
...
Consider[1] = function() --Target Ability Example
	...
	then
		...
		then
			if (
				(ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) and
					GetUnitToUnitDistance(npcBot, WeakestCreep) >= AttackRange + 300)
			then
	...
	then
		if (ManaPercentage > 0.4 and ability:GetLevel() >= 3 or ManaPercentage > 0.7 and ability:GetLevel() >= 2) and
			CreepHealth >= 150
		then
			if (WeakestEnemy ~= nil)
			then
	...
	then
		...
		then
			...
			then
				if (
					HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_PHYSICAL) and
						npcBot:GetMana() > ComboMana and #allys >= #enemys)
				then
